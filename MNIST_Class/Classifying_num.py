...
for epoch in range(epochs):
    ...
    loss = -np.mean(np.sum(train_labels_one_hot * np.log(a + 1e-8), axis=1))
    ...
    for j in range(len(weights)):
        dW = np.dot(activations[j].T, deltas[j]) / train_images.shape[0]
        db = np.sum(deltas[j], axis=0, keepdims=True) / train_images.shape[0]
